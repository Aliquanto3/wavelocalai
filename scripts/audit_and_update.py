...
def initialize_csv():
    ...
    if not DATASET_CSV_PATH.exists():
        headers = [
            "model_name", "ollama_tag", "disk_size_gb", "context_size", 
            "input_tokens", "output_tokens", 
            "ollama_ram_usage_gb", "ram_peak_gb", "ram_delta_gb",        
            "co2_kg", "duration_s", "status", "date"
        ]
...
def test_tool_capabilities(model_tag):
    ...
    try:
        resp = ollama.chat(model=model_tag, messages=messages, tools=[test_tool], options={'temperature': 0, 'num_ctx': 2048})
...
def test_language_support(model_tag, lang_code):
    ...
    msg = [{'role': 'user', 'content': f"Translate the word 'Hello' into {target}. Return only the translated word."}]
    try:
        resp = ollama.chat(model=model_tag, messages=msg, options={'temperature': 0, 'num_ctx': 2048})
...
def benchmark_single_pass(model_tag, context_window, output_token_limit=512):
    ...
    tracker = OfflineEmissionsTracker(country_iso_code="FRA", output_dir=str(LOGS_DIR), log_level="error")
    tracker.start()
    start_time = time.perf_counter()
    
    messages = [{'role': 'user', 'content': 'Write a very detailed technical guide about space travel physics.'}]
    ...
    try:
        ...
        if output_tokens == 0: output_tokens = len(resp.message.content.split())
...
def generate_markdown_report(db):
    ...
    with open(REPORT_MD_PATH, "w", encoding="utf-8") as f:
        f.write(f"# 📊 WaveLocalAI - Rapport de Stress Test\n")
        f.write(f"*Généré le : {datetime.now().strftime('%d/%m/%Y à %H:%M')}*\n")
        f.write(f"**RAM Totale Installée : {TOTAL_RAM_GB} GB**\n\n")
        f.write(f"> **Note :** Ce rapport affiche les performances maximales validées (sans swap). L'historique détaillé est dans `benchmarks_data.csv`.\n\n")
...
def main():
    ...
    parser.add_argument("--skip-tested", action="store_true", help="Ignorer les modèles déjà testés")
    ...
    for i, (name, data) in enumerate(items, 1):
        try:
            ...
            tag = data.get('ollama_tag')
            real_tag = tag if tag in installed else (f"{tag}:latest" if f"{tag}:latest" in installed else None)
            ...
            if size: data['size_gb'] = f"{size} GB"
            
            if test_tool_capabilities(real_tag):
                if "tools" not in data.get('capabilities', []): data.setdefault('capabilities', []).append('tools')
            else:
                if "tools" in data.get('capabilities', []): data['capabilities'].remove('tools')
            ...
            for ctx in CONTEXT_LEVELS:
                ...
                if res:
                    ...
                    if prev_ram_usage > 0 and current_ram < prev_ram_usage:
                        logger.warning(f"      📉 SWAP DETECTÉ. Arrêt. Max Validé : {bench_results[-1]['context_size']}")
                        break 
