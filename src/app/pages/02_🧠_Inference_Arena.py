...
# --- LAYOUT PRINCIPAL ---
tab_chat, tab_lab, tab_manager = st.tabs(
    ["💬 Chat Libre (Historique)", "🧪 Labo de Tests (One-Shot)", "⚙️ Gestion des Modèles"]
)
...
with tab_chat:
    ...
    with col_chat_params:
        ...
        model_map = (
            {get_friendly_name_from_tag(m["model"]): m["model"] for m in installed}
            if installed
            else {}
        )
        selected_friendly = st.selectbox(
            "Modèle actif", sorted(model_map.keys()), key="chat_model_select"
        )
    ...
    with col_chat_main:
        ...
        if prompt := st.chat_input("Discutez avec le modèle..."):
            ...
            with st.chat_message("assistant"):
                ...
                # ✅ SOLUTION : Utiliser un objet mutable (liste ou dict) au lieu de nonlocal
                state = {"current_text": ""}

                # Définition des callbacks UI
...
with tab_lab:
    ...
    with col_lab_config:
        ...
        lab_model_friendly = st.selectbox(
            "Modèle de Test", sorted(model_map.keys()), key="lab_model_select"
        )
    ...
    with col_lab_run:
        ...
        if st.button("🚀 Lancer le Test (One-Shot)", use_container_width=True):
            if lab_model_tag:
                ...
                # ✅ SOLUTION : Utiliser un dict mutable
                state = {"current_text": ""}
                ...
                # Sauvegarde pour métriques (colonne de droite)
    ...
    if installed_models:
        ...
        for m in installed_models:
            ...
            row = {
                ...
                "Max Contexte (tk)": benchmark_stats.get(
                    "tested_ctx", info.get("ctx", "N/A") if info else "N/A"
                ),
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "Documentation": st.column_config.LinkColumn(
                        "Lien Doc", display_text="Voir Fiche"
                    ),
                    "Taille": st.column_config.TextColumn(
                        "Disk (GB)", help="Espace disque occupé", width="small"
                    ),
                },
                hide_index=True,
            )
